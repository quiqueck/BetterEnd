...
/**
 * Binds BetterEnd's own {@link EndTinterKeys} to the tint sources that implement them. This is the extension
 * path every mod uses for a colour wover's built-in shapes cannot describe: declare a {@code TinterKey} in
 * common code, bind its client behaviour here, then attach {@code ClientBlockTraits.TINT} to the block.
 * <p>
 * Registered through the {@code wover.client.traits} entrypoint, so it runs in both the client and the datagen
 * launch.
 * <p>
 * Diverges from 26.x only in the lambda shape: 1.21.6's tint source is a {@code BlockColor}
 * ({@code getColor(state, level, pos, tintIndex)}), not a {@code BlockTintSource} ({@code color(state)}).
 */
...
public class EndClientTinters implements WoverClientTraitEntrypoint {
    ...
    public void registerClientTraits() {
        ...
        ClientTinterRegistry.register(
                ...
                (payload, block) -> (state, level, pos, tintIndex) ->
                        EndPortals.getColor(state.getValue(EndPortalBlock.PORTAL))
        );
    }
}
