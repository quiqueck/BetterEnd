...
public class TerrainGenerator {
    ...
    /**
     * The {@link WoverEndConfig} driving {@code biomeSource}, or {@code null} when the End of this world
     * is generated by a biome source BetterEnd does not own (vanilla {@code minecraft:the_end}, or any
     * other mod's End biome source).
     * <p>
     * Every part of BetterEnd's island terrain - the island layers, {@link #isLand}, the cave band - is
     * derived from this config, so "no config" means "not our End".
     */
    private static @Nullable WoverEndConfig endConfigOf(@Nullable BiomeSource biomeSource) {
        if (biomeSource instanceof BiomeSourceWithConfig<?, ?> bcl
                && bcl.getBiomeSourceConfig() instanceof WoverEndConfig cfg) {
            return cfg;
        }
        return null;
    }
    ...
    /**
     * Drops every piece of state {@link #initNoise} builds, putting the generator back into the same
     * "not initialised" state it has before the first End level is created.
     * <p>
     * Called when BetterEnd does not own a world's End (see {@link #endConfigOf}). All the read paths
     * already treat null island layers / null biome source / null config as "no BetterEnd terrain"
     * ({@link #isLand}, {@link #getSurfaceHeight}, {@link #getAverageDepth},
     * {@code EndCaveCarver.caveBandCeiling}), and {@code BETargetChecker.be_isTarget()} is set to false
     * alongside this, so none of BetterEnd's terrain hooks fire at all.
     * <p>
     * Resetting matters because all of this is <b>static</b>: an integrated server reuses one JVM across
     * world loads, so without the reset a vanilla-End world would silently keep the island layers and
     * config of whatever BetterEnd world was loaded before it.
     */
    private static void disableTerrainGenerator() {
        LOCKER.lock();
        try {
            config = null;
            biomeSource = null;
    ...
    /**
     * Builds the island terrain for an End driven by a BetterEnd/Wover End biome source.
     * <p>
     * When {@code biomeSource} carries no {@link WoverEndConfig} this is a no-op that disables the
     * generator instead ({@link #disableTerrainGenerator}) - BetterEnd does not own that End, so vanilla
     * generation must be left alone. It deliberately does not throw: this runs inside
     * {@code ServerLevel.<init>}, where any exception aborts the whole world load.
     */
    public static void initNoise(long seed, BiomeSource biomeSource, Sampler sampler) {
        final WoverEndConfig cfg = endConfigOf(biomeSource);
        if (cfg == null) {
            disableTerrainGenerator();
            return;
        }
    ...
    public static void fillTerrainDensity(double[] buffer, int posX, int posZ, int scaleXZ, int scaleY, int maxHeight) {
        ...
        LOCKER.lock();
        try {
            // Same defensive guard as isLand/getSurfaceHeight: the island layers are null whenever
            // BetterEnd does not own this End (see disableTerrainGenerator). fillSlice is gated on
            // be_isTarget() and so should never get here in that state, but a chunk worker NPE would be
            // far worse than leaving the column to vanilla, so bail out rather than dereference null.
    ...
    public static void onServerLevelInit(ServerLevel level, LevelStem levelStem, long seed) {
        if (level.dimension() == Level.END) {
            ...
            final BiomeSource endBiomeSource = chunkGenerator.getBiomeSource();
            if (chunkGenerator instanceof NoiseBasedChunkGenerator) {
                Holder<NoiseGeneratorSettings> sHolder = ((NoiseBasedChunkGeneratorAccessor) chunkGenerator)
                        .be_getSettings();
                if (endBiomeSource instanceof WoverEndBiomeSource bcl) {
            ...
            // A world whose End is generated by someone else's biome source (vanilla minecraft:the_end,
            // or another mod's) is not ours to shape. The be_setTarget(false) above already tells every
            // terrain hook to stand down; initNoise then disables the island generator as well, so
            // vanilla End generation runs untouched and - crucially - the world still LOADS. This code
            // runs inside ServerLevel.<init>, so throwing here (as it used to) took the entire server
            // down before it ever reached the "Done" line.
            if (endConfigOf(endBiomeSource) == null) {
                BetterEnd.LOGGER.warn(
                        "{} is generated by {}, which is not a BetterEnd End biome source. "
                                + "BetterEnd island terrain, cave biomes and End structures are DISABLED for this "
                                + "dimension; vanilla End generation is used instead. To get BetterEnd's End, create "
                                + "the world with the BetterEnd/Wover world preset (or set the dimension's biome "
                                + "source to wover:end).",
                        level.dimension().location(),
                        endBiomeSource.getClass().getSimpleName()
                );
            }
